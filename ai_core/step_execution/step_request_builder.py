...
class StepRequestBuilder:
    def build(self, *, compiled_step: dict[str, Any], task_session_id: str, task_context: dict[str, Any] | None = None, resolved_bindings: dict[str, Any] | None = None) -> dict[str, Any]:
        return {
            ...
            "execution_known": compiled_step.get("execution_known") if isinstance(compiled_step.get("execution_known"), dict) else {},
            "semantic_known": compiled_step.get("semantic_known") if isinstance(compiled_step.get("semantic_known"), dict) else {},
            "task_metadata": compiled_step.get("task_metadata") if isinstance(compiled_step.get("task_metadata"), dict) else {},
