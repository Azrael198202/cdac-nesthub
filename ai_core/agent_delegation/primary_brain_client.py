...
class PrimaryBrainDelegationClient:
    ...
    def _try_self_contained_runtime_observation(self, request: AgentExecutionRequest) -> AgentExecutionResult | None:
        """No fixed runtime-observation shortcut is performed in ai_core.

        Runtime values must be supplied by a registered/generated capability, a
        delegated participant, or the configured model/tool route.  Keeping this
        hook disabled avoids embedding domain-specific behavior in the universal
        runtime brain and ensures create-agent / execute-agent requests go
        through the normal planning and evidence path.
        """
        return None
