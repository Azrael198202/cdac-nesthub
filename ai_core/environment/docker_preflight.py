...
class DockerPreflight:
    """Generic Docker preflight check used before isolated runtime execution.
    ...
    def check(self, *, image: str | None = None, pull_if_missing: bool = False, timeout_seconds: int = 120) -> dict[str, Any]:
        ...
        if not docker:
            return asdict(DockerPreflightResult(
                status="unavailable",
        ...
        return asdict(DockerPreflightResult(
            ...
            reason="Docker preflight passed." if can_run else "Docker preflight failed or image is unavailable.",
        ))
