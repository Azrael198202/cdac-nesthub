...
class ContinuationEngine:
    ...
    def build_pending_action(self, node_id: str, result: dict[str, Any], state: dict[str, Any] | None = None) -> dict[str, Any] | None:
        ...
        if human_interactions:
            runtime_request = self._runtime_parameter_request(human_interactions)
            if runtime_request is not None:
                return {
                    "kind": "collect_runtime_parameters",
                    "node_id": node_id,
                    "request": runtime_request,
                    "result": result,
                    "message": runtime_request.get("message") or "Runtime input is required before execution can continue.",
                }
    ...
    def _runtime_parameter_request(self, interactions: list[dict[str, Any]]) -> dict[str, Any] | None:
        """Preserve runtime parameter contracts generated by validation/execution.

        These contracts already contain executable field names and merge metadata.
        Sending them through the generic question generator can turn a field
        object into a string label, which prevents the UI from resuming with a
        clean `{field: value}` payload.
        """
        fields: list[dict[str, Any]] = []
        source_type = ""
        step_id = ""
        message = "Please provide the required runtime input values and resume the workflow."
        title = "Required runtime input"
        for interaction in interactions or []:
            if not isinstance(interaction, dict):
                continue
            interaction_type = str(interaction.get("type") or interaction.get("kind") or "")
            candidate_fields = interaction.get("fields") if isinstance(interaction.get("fields"), list) else []
            if interaction_type in {"collect_runtime_parameters", "runtime_parameter_input", "uploaded_artifact_parameters"} or candidate_fields:
                source_type = interaction_type or source_type or "collect_runtime_parameters"
                step_id = str(interaction.get("step_id") or step_id or "")
                message = str(interaction.get("message") or interaction.get("reason") or message)
                title = str(interaction.get("title") or title)
                for raw in candidate_fields:
                    if isinstance(raw, dict):
                        name = str(raw.get("field") or raw.get("name") or raw.get("source_field") or "").strip()
                        if not name:
                            continue
                        field = dict(raw)
                        field.setdefault("field", name)
                        field.setdefault("name", name)
                        field.setdefault("label", raw.get("label") or name)
                        field.setdefault("type", raw.get("input_type") or raw.get("type") or "text")
                        field.setdefault("required", raw.get("required", True))
                        fields.append(field)
                    elif isinstance(raw, str) and raw.strip():
                        name = raw.strip()
                        fields.append({"field": name, "name": name, "label": name, "type": "text", "required": True})
        if not fields:
            return None
        return {
            "kind": source_type or "collect_runtime_parameters",
            "type": source_type or "collect_runtime_parameters",
            ...
            "resume_strategy": {"type": "resume_workflow", "merge_target": "runtime_parameters"},
        }
