    ...
    def prompt_template(self, stage_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "runtime_rules": [
                ...
                "agent_action_planning MUST use the agent action prompt contract: content + target + fixed action list + selection rules. It then ranks fixed execution options: llm_generate, generate_code, generate_shell, call_api_no_key, call_api_with_key, web_query, use_existing_tool, use_external_skill, use_local_knowledge, generate_complex_tool, compose_static_response, ask_user, no_op.",
                ...
                "For web_query or API actions, execution_preparation must prepare concrete targets/endpoints or a discovery contract before execution; it must not rewrite the selected action to llm_generate.",
