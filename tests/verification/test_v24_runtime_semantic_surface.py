...
def test_compact_duration_is_converted_to_request_surface_and_not_matched_directly():
    ...
    request = "Please prepare a 3-unit plan for AlphaPlace with ordered stops."
    ...
    good = {"title": "AlphaPlace plan", "text_excerpt": "AlphaPlace 3-unit plan with ordered stops, time blocks, source notes, section summaries, verified records, reusable planning details, and enough body material for runtime validation."}
    ...
    assert "3-unit" in result["aggregate_coverage"]["matched"]["period"]
...
def test_direct_answer_builder_outputs_surface_known_parameter():
    ...
    result = builder.build(
        candidates=[{"title": "AlphaPlace guide", "url": "https://example.test/a", "text_excerpt": "AlphaPlace 5-unit plan with multiple ordered sections, verified records, reusable details, enough body material for runtime validation, and a clear non-compact period surface.", "score": 1}],
        payload={"user_input": "Create a 5-unit plan for AlphaPlace.", "known": {"target": "AlphaPlace", "period": "P5D"}},
        capability="generic_research",
    )
    assert result is not None
    assert result["data"]["known_parameters"]["period"] == "5-unit"


def test_semantic_surface_normalizer_omits_compact_token_from_aliases():
    normalizer = RuntimeSemanticSurfaceNormalizer()
    aliases = normalizer.semantic_aliases("P3D", "make a 3-unit output")
    lowered = [x.lower() for x in aliases]
    assert "p3d" not in lowered
    assert "3-unit" in lowered
