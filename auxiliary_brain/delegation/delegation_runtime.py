...
class AgentDelegationRuntime:
    ...
    async def _resume_registered_tool_confirmation(
        ...
        *,
        ...
        if isinstance(provided_inputs, dict):
            runtime_parameters.update(self._expand_submitted_runtime_parameters(pending, provided_inputs))
    ...
    def _normalize_runtime_parameter_value(self, value: Any) -> Any:
        """Normalize UI-submitted parameter values before storing them.

        The runtime input UI may submit list fields as arrays.  Older browser
        states, manual tests, or copied previews can submit a JSON-encoded list
        as a single string inside that array.  Store one canonical value so the
        confirmation preview and the executable payload read the same parameter
        map.  This is structural normalization only; it does not know any
        capability-specific parameter names.
        """
        if isinstance(value, list):
            normalized: list[Any] = []
            for item in value:
                ...
                if isinstance(item, str):
                    ...
                    if text.startswith("[") and text.endswith("]"):
                        ...
                        if isinstance(parsed, list):
                            normalized.extend([x for x in parsed if x not in (None, "", [], {})])
                            continue
    ...
    def _expand_submitted_runtime_parameters(self, pending_action: dict[str, Any] | None, provided_inputs: dict[str, Any] | None) -> dict[str, Any]:
        """Expand submitted UI fields into a canonical task-run parameter map.

        The browser may submit values as ``step.field``, ``participant.field``,
        plain ``field``, or aliases generated by an older interaction request.
        Resume must not depend on one spelling.  This function uses the active
        pending action contract to fan out each submitted value into the plain,
        dotted, and underscored aliases for the declared participant and
        parameter.  It is generic and schema-contract driven.
        """
        if not isinstance(provided_inputs, dict):
            return {}
        out: dict[str, Any] = {}
        for k, v in provided_inputs.items():
            normalized = self._normalize_runtime_parameter_value(v)
            if normalized not in (None, "", [], {}):
                out[k] = copy.deepcopy(normalized)
        ...
        if isinstance(request.get("fields"), list):
            fields.extend([f for f in request.get("fields") if isinstance(f, dict)])
        if isinstance(pending_action.get("fields"), list):
            fields.extend([f for f in pending_action.get("fields") if isinstance(f, dict)])

        lower_lookup = {str(k).casefold(): k for k in out.keys()}
        for field in fields:
            ...
            if participant_id:
                candidates.extend([f"{participant_id}.{pname}", f"{participant_id}_{pname}"])
            ...
            for candidate in candidates:
                if not candidate:
                    continue
                if candidate in out and out[candidate] not in (None, "", [], {}):
                    value = self._normalize_runtime_parameter_value(out[candidate])
                    found = True
                    break
                matched = lower_lookup.get(candidate.casefold())
                if matched is not None and out.get(matched) not in (None, "", [], {}):
                    value = self._normalize_runtime_parameter_value(out[matched])
                    found = True
                    break
            if not found:
                continue
            out.setdefault(pname, copy.deepcopy(value))
            if participant_id:
                out.setdefault(f"{participant_id}.{pname}", copy.deepcopy(value))
                out.setdefault(f"{participant_id}_{pname}", copy.deepcopy(value))
        return out
    ...
    async def resume_task(self, run_payload: dict[str, Any], task_graph: dict[str, Any], participants: list[dict[str, Any]], provided_inputs: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        pending = run_payload.get("pending_action") if isinstance(run_payload.get("pending_action"), dict) else {}
        if str(pending.get("kind") or "") in {"agent_parameter_collection", "studio_pre_execution_runtime_parameters"} and not run_payload.get("agent_results"):
            # Top-level parameter collection before any participant has
            # executed may safely start a fresh task run with the completed
            # runtime parameter map.  This includes the Studio pre-execution
            # parameter gate and the delegation-runtime parameter gate.  If
            # agent_results already exist, this is a participant-level pause and
            # must be resumed below without re-running completed upstream
            # participants.
    ...
    def _participant_parameter_aliases(self, participant: dict[str, Any]) -> set[str]:
        ...
        for value in (
            ...
            participant.get("original_participant_id"),
            participant.get("durable_participant_id"),
            participant.get("declared_participant_id"),
            participant.get("step_id"),
            participant.get("compiled_step_id"),
            participant.get("source_step_id"),
            participant.get("declared_step_id"),
        ):
    ...
    def _participant_all_input_fields(self, participant: dict[str, Any]) -> list[dict[str, Any]]:
        """Return the full user-facing input surface for a participant.

        Missing-input prompts must not be rebuilt one required key at a time.
        When a required value blocks execution, the UI should show every
        schema-declared runtime field for that same participant, marking only
        required fields as blocking.  This is schema/contract driven and remains
        capability-agnostic.
        """
        ...
        def add(field: dict[str, Any]) -> None:
            if not isinstance(field, dict):
                return
            pname = str(field.get("parameter_name") or field.get("name") or field.get("field") or field.get("key") or "").strip()
            if "." in pname:
                pname = pname.rsplit(".", 1)[-1]
            if not pname:
                return
            owner = str(field.get("participant_id") or self._participant_identity(participant) or self._participant_name(participant) or "")
            key = (owner.casefold(), pname.casefold())
            if key in seen:
                return
            seen.add(key)
            enriched = dict(field)
            enriched.setdefault("parameter_name", pname)
            enriched.setdefault("participant_id", self._participant_identity(participant))
            enriched.setdefault("participant_name", self._participant_name(participant))
            fields.append(enriched)

        # Runtime-registered tools have the most concrete executable schema.
        try:
            for field in self.registered_tool_parameter_bridge.input_fields_from_schema(participant=participant):
                add(field)
        except Exception:
            pass
        ...
        for item in contract.get("parameters") or []:
            ...
            add({
                ...
                "label": f"{pname} / {item.get('label') or name}" if pname else str(item.get("label") or name),
                "message": str(item.get("description") or item.get("message") or f"Please provide {name}."),
                "input_type": str(item.get("input_type") or item.get("type") or "text"),
                "required": required,
                "collection_mode": item.get("collection_mode") or ("repeat_until_done" if str(item.get("type") or "").casefold() in {"list", "array"} else "single_value"),
                "runtime_required": required,
                "blocking": required,
                "execution_required": required,
                "source_schema_type": item.get("source_schema_type") or item.get("type"),
            })

        for field in self.parameter_contract_service.to_missing_input_fields(participant):
            add(field)
        for field in self._capability_required_fields(participant):
            add(field)
        return fields
    ...
    def _scoped_runtime_parameters_from_values(self, participant: dict[str, Any], runtime_parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in runtime_parameters.items():
            ...
            # Execution-control values belong to the run, not to the tool's
            # business schema. Preserve them on the participant so confirmation
            # resume can continue, while _registered_tool_business_input() still
            # filters them before invoking the capability.
            if token in {"approval_confirmed", "remember_approval", "confirm", "confirmed", "approval", "approved"}:
                out[text_key] = copy.deepcopy(value)
                continue
            matched = False
            if "." in text_key:
                prefix, tail = text_key.rsplit(".", 1)
                if prefix.casefold() in aliases or self._parameter_alias_token(prefix) in aliases:
                    out.setdefault(str(tail).strip(), copy.deepcopy(value))
                    matched = True
            if matched:
                continue
            for alias in aliases:
                ...
                if lower.startswith(dotted):
                    out.setdefault(text_key[len(dotted):], copy.deepcopy(value))
                    matched = True
                    break
                if token.startswith(underscored):
                    ...
                    out.setdefault(field, copy.deepcopy(value))
                    matched = True
                    break
            if matched:
                continue
    ...
    def _collect_missing_agent_parameter_fields(self, participants: list[dict[str, Any]], dependency_plan: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        for participant in participants:
            ...
            all_fields = self._participant_all_input_fields(participant)
            ...
            for field in all_fields:
                ...
                if self._can_defer_field_to_dependency_output(participant, field, dependency_plan or {}):
                    continue
                if self._is_blocking_agent_parameter_field(participant, field):
                    blocking_fields.append(field)

            # Only pause when at least one required field is really missing.
            # When pausing, show the full unbound schema surface for this
            # participant once: required fields block; optional fields are
            # visible and may be left blank.
            if not blocking_fields:
                continue

            blocking_names = {
                str(f.get("parameter_name") or f.get("name") or f.get("field") or "").rsplit(".", 1)[-1].casefold()
            ...
            for field in all_fields:
                ...
                if not pname:
                    continue
                key = (owner, pname.casefold())
                if key in seen:
                    continue
                seen.add(key)
                enriched = dict(field)
                is_blocking = pname.casefold() in blocking_names
                enriched["required"] = bool(enriched.get("required", is_blocking))
                enriched["blocking"] = is_blocking
                enriched["execution_required"] = is_blocking
                enriched["runtime_required"] = is_blocking
                fields.append(enriched)
        return fields
    ...
    def _runtime_field_already_bound(self, participant: dict[str, Any], field: dict[str, Any]) -> bool:
        ...
        if not isinstance(values, dict):
            return False
        aliases = [name]
        for prefix in self._participant_parameter_aliases(participant):
            if prefix:
    ...
    async def _execute_registered_tool_capability(self, *, participant: dict[str, Any], task_name: str, completed_results: list[Any] | None = None, dependency_plan: dict[str, Any] | None = None) -> AgentExecutionResult | None:
        ...
        if missing:
            ...
            input_data, missing = self._separate_registered_tool_approval_controls(input_data=input_data, missing=missing, approval_confirmed=policy_auto_approved)
        if missing:
            raw_fields = self._participant_all_input_fields(participant)
            ...
            for field in raw_fields:
                ...
                enriched = dict(field)
                is_blocking = pname.casefold() in missing_set
                enriched["blocking"] = is_blocking
                enriched["execution_required"] = is_blocking
                enriched["runtime_required"] = is_blocking
                enriched["required"] = bool(enriched.get("required", is_blocking))
