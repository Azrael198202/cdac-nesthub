...
class ExternalSolutionDiscoveryEngine:
    ...
    def __init__(self) -> None:
        ...
        self.evidence_optimizer = WebEvidenceOptimizer()
    ...
    def _queries(self, request: dict[str, Any]) -> list[str]:
        """Return compact evidence queries generated by WebEvidenceOptimizer.

        The old path used the whole user prompt, which made public search noisy
        and often returned zero kept results.  The optimizer first extracts a
        small identity/objective string, then creates purpose-scoped queries.
        """
        ...
        planned = self.evidence_optimizer.plan_queries(
            user_input=str(request.get("user_input") or ""),
            capability=str(request.get("capability") or ""),
            objective=str(step.get("objective") or step.get("action") or ""),
            known=known,
        )
        queries = [str(item.get("query") or "").strip() for item in planned if isinstance(item, dict)]
        queries = [q for q in queries if q]
        if queries:
            return queries[:6]
        return ["runtime capability official documentation example validation"]
    ...
    def _finalize(
        ...
        *,
        ...
        optimized_evidence = self.evidence_optimizer.optimize(
            user_input=str(request.get("user_input") or ""),
            capability=str(request.get("capability") or ""),
            objective=str((request.get("source_step") or {}).get("objective") or "") if isinstance(request.get("source_step"), dict) else "",
            known=(request.get("runtime_request_semantics") or {}).get("known_parameters", {}) if isinstance(request.get("runtime_request_semantics"), dict) else {},
            search_results=web_results,
            documents=documents,
            max_items=8,
        )
        status = "success" if any([web_results, documents, repositories, models]) else "no_external_evidence"
        if optimized_evidence.get("status") == "verified":
            status = "success"
        record = {
            "status": status,
            "request": request,
